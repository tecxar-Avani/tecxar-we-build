...
import {
  ...
  IDeleteGroups,
  IGroupBoxData,
} from "@/interfaces/boxesgroups.interface";
...
class GroupService {
  ...
  public async createGroup(groupData: IGroups): Promise<IGroups | null> {
  ...
  public async createGroupBoxes(
    ...
    if (getGroupId) {
      const createGroupBoxData: any[] | null = await this.boxGroups.bulkCreate(
        boxes
      );
    }
  ...
  public async getGroupBoxes(buildId: number): Promise<IGroupBoxData[] | any> {
  ...
  public async getGroupByBox(boxId: number): Promise<IGroupBoxData | any> {
    const getGroupBox = await this.boxGroups.findAll({
      attributes: ["group_id"],
      where: { box_id: boxId },
      group: ["group_id"],
      raw: true,
  ...
  public async deleteGroupById(
    ...
    const getGroupsBoxes: IBoxesGroups[] | null = await this.boxGroups.findAll({
      where: { group_id: { [Op.in]: [groupId] } },
    });
    if (!getGroupsBoxes) return false;
    await this.boxGroups.destroy({ where: { group_id: groupId } });
    const getGroups: IGroups[] | null = await this.group.findAll({
      where: { id: { [Op.in]: [groupId] } },
    });
  ...
  public async getGroupByBuildId(
    ...
    const getBox = await this.boxes.findAll({
      attributes: ["id"],
      where: { build_id: buildId },
      raw: true,
      logging: console.log,
